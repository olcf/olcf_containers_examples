...
int main(int argc, char *argv[])
{
	...
	gpuErrorCheck( hipSetDevice(rank % num_devices) );
	...
	for(int i=0; i<=27; i++){
		...
		double *d_A;
		gpuErrorCheck( hipMalloc(&d_A, N*sizeof(double)) );
		gpuErrorCheck( hipMemcpy(d_A, A, N*sizeof(double), hipMemcpyHostToDevice) );
